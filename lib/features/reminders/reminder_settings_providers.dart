...
class ReminderSettingsNotifier extends StateNotifier<ReminderSettings> {
  ReminderSettingsNotifier(this._notificationService) : super(ReminderSettings.defaults) {
    _load();
  }
  ...
  Future<void> _load() async {
    ...
    state = loaded;
    if (loaded.enabled) {
      await _notificationService.scheduleDailyReminder(hour: loaded.hour, minute: loaded.minute);
    }
  }
  ...
  Future<bool> setEnabled(bool enabled) async {
    ...
    if (enabled) {
      await _notificationService.scheduleDailyReminder(hour: state.hour, minute: state.minute);
    } else {
...
final reminderSettingsProvider = StateNotifierProvider<ReminderSettingsNotifier, ReminderSettings>((ref) {
  return ReminderSettingsNotifier(ref.watch(notificationServiceProvider));
});
