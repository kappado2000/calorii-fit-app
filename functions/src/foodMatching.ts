...
import { FoodProductResult, kcalPer100g, extractMicronutrients, numberOrNull } from "./foodProduct";
 ...
 */
export function wordsFuzzyMatch(a: string, b: string): boolean {
  if (a === b) return true;
...
function wordsOf(value: string): string[] {
  return foldDiacritics(value).split(/[^a-z0-9]+/).filter(Boolean);
}
 ...
 * external results.
 */
export function matchGenericFoodItems(query: string): FoodProductResult[] {
  const queryWords = wordsOf(query);
  if (queryWords.length === 0) return [];

  return genericFoodsTable
    .filter((item) => {
      const itemWords = wordsOf(item.name);
...
export function rerankByNamePrefix(products: FoodProductResult[], query: string): FoodProductResult[] {
...
/**
 * A result only counts as a real match to [label] if they share at least
 * one fuzzy-matching word — the reranking above still lets through a
 * "closest available" OFF hit that doesn't actually name the same food
 * (full-text search matches ingredients/categories too), and applying a
 * wrong product's macros with false confidence is worse than falling back
 * to the existing category-average estimate.
 */
function sharesAWord(label: string, name: string): boolean {
  const labelWords = wordsOf(label);
  const nameWords = wordsOf(name);
  return labelWords.some((lw) => nameWords.some((nw) => wordsFuzzyMatch(lw, nw)));
}

/**
 * Best-effort nutrition lookup for a food label identified from a photo
 * (see analyzePhoto.ts) — reuses the exact same matching the manual/barcode
 * search path (searchFoods) already relies on, so a photo-identified
 * "piept de pui la grătar" resolves against the same curated table / Open
 * Food Facts data a manual search for the same words would. Returns null
 * (never throws) on no match, low-confidence match, or any lookup failure —
 * callers must treat this as purely additive and keep working without it.
 */
export async function findBestNutritionMatch(label: string): Promise<FoodProductResult | null> {
  const genericMatches = matchGenericFoodItems(label);
  if (genericMatches.length > 0) return genericMatches[0];

  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 3500);
    const response = await fetch(buildSearchUrl(label), { signal: controller.signal });
    ...
    const candidates = (data.hits ?? [])
      .map(toResult)
      .filter((p): p is FoodProductResult => p !== null && sharesAWord(label, p.name));
    if (candidates.length === 0) return null;

    return rerankByNamePrefix(candidates, label)[0];
  } catch {
    return null;
  }
}
